...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        ...
        if($path == '') {
            ...
            $statement->execute(array($_POST['p_name'], $_POST['p_old_price'], $_POST['p_current_price'], $_POST['p_qty'], $_POST['p_description'], $_POST['p_short_description'], $_POST['p_feature'], $_POST['p_condition'], $_POST['p_return_policy'], $is_featured, $is_active, $_POST['ecat_id'], $_REQUEST['id']));
        } else {
            ...
            $statement->execute(array($_POST['p_name'], $_POST['p_old_price'], $_POST['p_current_price'], $_POST['p_qty'], $final_name, $_POST['p_description'], $_POST['p_short_description'], $_POST['p_feature'], $_POST['p_condition'], $_POST['p_return_policy'], $is_featured, $is_active, $_POST['ecat_id'], $_REQUEST['id']));
        }
...
<main class="flex-grow p-6 md:p-8">
    <form action="" method="post" enctype="multipart/form-data" class="max-w-[1400px] mx-auto">
        ...
        <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
                <span class="text-slate-500 text-[10px] uppercase tracking-[0.2em] font-bold">Admin > Inventory > Edit</span>
                <h1 class="text-3xl font-extrabold tracking-tight mt-1">Edit: <?php echo htmlspecialchars($p_name); ?></h1>
            </div>
            <div class="flex gap-4 w-full md:w-auto">
                <a href="product.php" class="px-6 py-2.5 rounded-lg text-[#0052CC] text-sm font-bold border border-[#0052CC]/20 hover:bg-slate-100 transition-all text-center flex-1 md:flex-none">Discard Changes</a>
                <button type="submit" name="form1" class="px-8 py-2.5 rounded-lg bg-[#0052CC] hover:bg-blue-700 text-white text-sm font-bold shadow-md transition-all flex-1 md:flex-none">Save Changes</button>
            </div>
        </div>
        ...
        <?php if($success_message): ?>
            <div class="bg-green-50 text-green-600 px-4 py-3 rounded-lg mb-6 text-sm font-bold border border-green-200 flex justify-between items-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-8 space-y-8">
                ...
                <section class="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-2">
                            ...
                            <select name="tcat_id" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 text-sm outline-none">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="mcat_id" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 text-sm outline-none">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="ecat_id" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 text-sm outline-none">
                ...
                <section class="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
                    <h2 class="text-lg font-bold mb-6 flex items-center gap-2"><span class="material-symbols-outlined text-[#0052CC]">info</span> Product Information</h2>
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Product Name *</label>
                            <input name="p_name" value="<?php echo htmlspecialchars($p_name); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 outline-none font-medium" type="text" required/>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Short Description</label>
                            <textarea name="p_short_description" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 outline-none" rows="2"><?php echo htmlspecialchars($p_short_description); ?></textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Full Description</label>
                            <textarea name="p_description" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 outline-none" id="editor1" rows="5"><?php echo htmlspecialchars($p_description); ?></textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Features / Specifications</label>
                            <textarea name="p_feature" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 outline-none" id="editor2" rows="4"><?php echo htmlspecialchars($p_feature); ?></textarea>
                        </div>
                    </div>
                </section>
            </div>
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <section class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="mb-4">
                        <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-2">Current Featured Image</label>
                        <div class="aspect-video w-full rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-center p-2">
                            <?php if($p_featured_photo == ''): ?>
                                <span class="text-slate-400 text-sm font-bold">No Image Uploaded</span>
                            <?php else: ?>
                                <img src="../assets/uploads/<?php echo $p_featured_photo; ?>" class="max-w-full max-h-full object-contain mix-blend-multiply">
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="mb-6">
                        <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-2">Replace Image (Optional)</label>
                        <div class="relative group cursor-pointer border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
                            ...
                            <div class="p-6 flex flex-col items-center justify-center text-center">
                                <span class="material-symbols-outlined text-3xl text-[#0052CC] mb-1">cloud_upload</span>
                                <p class="text-xs font-bold text-slate-600">Upload New Image</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div class="space-y-2">
                            ...
                            <input name="p_old_price" value="<?php echo htmlspecialchars($p_old_price); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-3 text-sm outline-none" type="text"/>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Current Price *</label>
                            <input name="p_current_price" value="<?php echo htmlspecialchars($p_current_price); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-3 text-sm font-bold outline-none" type="text" required/>
                        </div>
                    </div>
                    <div class="space-y-2 mb-6">
                        <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Stock Quantity *</label>
                        <input name="p_qty" value="<?php echo htmlspecialchars($p_qty); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 text-sm font-bold outline-none" type="number" required/>
                    </div>
                    <div class="pt-4 border-t border-slate-100 space-y-4 mb-6">
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Sizes</label>
                            <select name="size[]" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-sm outline-none h-24" multiple="multiple">
                                <?php
                        ...
                        <div class="space-y-2">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Colors</label>
                            <select name="color[]" class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-sm outline-none h-24" multiple="multiple">
                                <?php
                    ...
                    <div class="pt-4 border-t border-slate-100 space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input name="p_is_featured" value="1" class="w-5 h-5 rounded text-[#0052CC] border-slate-300 focus:ring-[#0052CC]" type="checkbox" <?php if($p_is_featured == 1) echo 'checked'; ?>/>
                            <span class="text-sm font-semibold text-slate-700 group-hover:text-[#0052CC]">Featured on Homepage?</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input name="p_is_active" value="1" class="w-5 h-5 rounded text-[#0052CC] border-slate-300 focus:ring-[#0052CC]" type="checkbox" <?php if($p_is_active == 1) echo 'checked'; ?>/>
                            <span class="text-sm font-semibold text-slate-700 group-hover:text-[#0052CC]">Active (Visible in Store)?</span>
                        </label>
                    </div>
                </section>
            </div>
